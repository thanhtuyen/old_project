...
function Jpostal ( i_JposDb ) {
    ...
    this.displayAddress = function () {
        ...
        var address_city_under_section = '';
        for ( var key in this.options.address ) {
            var s = this.formatAddress( this.options.address[key], this.address );
            if(key === '#address_city'){
                if(s.indexOf('市') !== -1){
                    var split_address_city = s.split('市');
                    s = split_address_city[0]+'市';
                    if(typeof split_address_city[1] !== "undefined"){
                        address_city_under_section = split_address_city[1];
                    }else{
                        address_city_under_section = '';
                    }
                }
            }
            if(key === '#address_detail' && address_city_under_section !== ''){
                s = address_city_under_section + s;
            }
            console.log(key+':'+s);
            $(key).val( s );
        }
