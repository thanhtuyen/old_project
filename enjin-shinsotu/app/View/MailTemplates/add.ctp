...
<?php echo $this->element('back_nav', array('text' => isset($form_copy) ? __('テンプレート複製') : __('テンプレート作成')))?>
											...
											<tr>
												...
												<div class="col-lg-10 m-t-sm pull-right row text_right replace-src">
													<?php echo $this->Form->input('replace_word',array(
														...
														<btn class="btn btn-primary btn-sm replace-btn">挿入</btn>
													</div>
												</div>
											</div>
...
<?php echo $this->element('b_back_nav')?>
							...
							<script>
								$(function(){
									...
									$('.replace-btn').click(function(){
										...
										if(rep = dest.val().replace(/\#(.+?)\#/,src.text())){
											dest.val(rep);
										}else{
											dest.val(src.text());
										}
                  ...
                  <?php if(isset($form_copy)){
                  	echo "loadTargetByPurposeId('".$this->request->data['MailTemplate']['purpose_id']."','#MailTemplateSelectName','".$this->request->data['MailTemplate']['selectName']."');";
                  }else{
                  	echo "loadTargetByPurposeId('0','#MailTemplateSelectName',0);";
                  }?>
...
<div class="mailTemplates form">
	...
	<fieldset>
		<legend><?php echo __('Add Mail Template'); ?></legend>
		<?php
