...
abstract class Enjin_Controller extends REST_Controller {
	...
	protected function chekapikey()
	{
		...
		if( $this->input->server("REQUEST_METHOD") == "GET" ){
			...
			$rec_company_id = $var_array["rec_company_id"];
                ...
                if( !isset($rec_company_id) or !is_numeric($rec_company_id)){
                    //error response
	...
	protected function getCanCandidateId(){
		$candidate = $this->getCanCandidate();
		if (isset($candidate['id'])) return $candidate['id'];
		return false;
	}

	/**
	 * Get rec_company_id from session
	 * @return mixed
	 */
	protected function getRecCompanyIdFromSession(){
		$candidate = $this->getCanCandidate();
		if (isset($candidate['rec_company_id'])) return $candidate['rec_company_id'];
		return false;
	}

	/**
	 * Get the candidate object in session
	 * @return mixed
	 */
	protected function getCanCandidate(){

		return $this->session->userdata('canCandidate');
	}

	/**
	 * Set candidate object to Session
	 * @param $candidate
	 * @return mixed
	 * @internal param $id
	 */
	protected function setCanCandidate($candidate){
		return $this->session->set_userdata('canCandidate', $candidate);
	}
	...
	protected function getCurrentObject(){
		return $this->session->userdata('currentObject');
	}

	/**
	 * Set ObjectID and table to Session
	 * The current object they locked to edit
	 * @param $id
	 * @param $table
	 * @return mixed
	 */
	protected function setCurrentObject($id, $table){
		return $this->session->set_userdata('currentObject', array('id'=>$id, 'table'=>$table));
	}
	...
	protected function deleteCurrentObjectId(){
		return $this->session->unset_userdata('currentObject');
	}
